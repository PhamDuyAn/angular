...
public class ProjectsController {
	...
	public ResponseEntity<?> updateStaff(@PathVariable("id") UUID id, @RequestBody Projects projects) {
		logger.info("Updating Projects with id {}", id);

		Projects currentpro = projectsService.findByProject_id(id);

		if (currentpro == null) {
			logger.error("Unable to update. Projects with id {} not found.", id);
	...
	public ResponseEntity<?> deleteStaff(@PathVariable("id") UUID id) {
		...
		if (projects == null) {
			...
			return new ResponseEntity(new CustomErrorType("Unable to delete. Projects with id " + id + " not found."),
					HttpStatus.NOT_FOUND);
		}
